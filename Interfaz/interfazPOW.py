...
class InterfazPow():
    ...
    def construirVentana(self, numNodos):
        ...
        self.infoConsensoLb = Label(self.controlNCFrame, text="Consenso de la red: POW", font=("Arial", 12))
        ...
        for i in range(1, numNodos + 1):
            ...
            minarNbtn = Button(nodoGB, text="Minar", command= lambda n=nodo: threading.Thread(target=self.minar, args=(n)).start())
            Tooltip(minarNbtn, "Botón para mandar a minar al nodo")
            minarNbtn.grid(row=1, column=1)
            ...
            visaalizarNbtn.grid(row=2, column=1)
    ...
    """
    Metodo para el boton de minar.
    Manda minar al nodo seleccionado. Para esta seleccion se realiza pansando el número del nodo por paremetro
    """
    def minar(self, numNodo):
        self.imprimirConsola("Mandando minar al nodo " + numNodo)
        dirNodo = self.conf["Direccion Nodo"]["Nodo" + numNodo]
        dirNodo = "http://" + dirNodo

        try:
            respuesta = requests.get(dirNodo+"/minar")
            respuesta.raise_for_status()

            respuesta = "Bloque minado con exito"
        except Exception as e:
            respuesta = self.procesarError(e)
